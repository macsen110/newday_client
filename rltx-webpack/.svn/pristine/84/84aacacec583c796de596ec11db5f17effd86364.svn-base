  ...
  <div id="list" class="list-page">
    <div class="list-wrap">
      <div class="page-header clearfix">
        <h2 class="page-title fl">车辆管理</h2>
      ...
      <div class="block">
        <div class="table-opr clearfix">
          <div class="mini-search fr" id="miniSearch">
            <v-minTableSearch :getData="getData" :searchFields="searchFields" :searchModel="searchModel" :isShow="false" ref="tableSearch"></v-minTableSearch>
          </div>
        </div>
        <!-- fix-table-wrap -->
          ...
          <table class="rl-fix-table table-bordered">
            ...
            <tr v-for="list in listData" :data-error="list.error">
              ...
              <td class="fix-col">
                <div class="cell" style="width: 100px">
                  <el-select placeholder="请选择" v-model="operValues[list.code]">
                    <el-option v-if="list.blacklist === 'false'" :key="'加入黑名单'" :label="'加入黑名单'" :value="'加入黑名单'">
                      <div @click="handleDisabled(list, 'true')">
                        <a href="JavaScript:;">加入黑名单</a>
                      </div>
                    </el-option>
                    <el-option v-if="list.blacklist === 'true'" :key="'移出黑名单'" :label="'移出黑名单'" :value="'移出黑名单'">
              ...
              <td v-for="(column,index) in columnsData">
                <template v-if="column.detailLinkVisiable==='true' && detailUrl">
                  <a href="javascript:;" class="blue" @click="openWindow(`${detailUrl}?code=${list.code}&editable=false`, `${objectName}详情`)">
...
<script type="text/ecmascript-6">
  ...
  import { getSearch, getColumn, list, disableTruck } from '../../api/TruckManagementService';
  ...
  export default {
    ...
    data() {
      return {
        ...
        topDistance: '',
        operValues: {},
        detailUrl: '/truckManagement/detail.html'
      };
    },
    methods: {
      resetScrollBar() {
      ...
      handleDisabled(row, disabled) {
        ...
        let message = '您确认将该车辆加入黑名单吗？加入黑名单后，该车辆将无法登陆平台';
        if (!disabled) {
          message = '您确认将该车辆移出黑名单吗？移出黑名单后，该车辆将可以正常登陆平台';
        }
        this.$msgbox({
          title: '消息',
          message: h('p', null, [
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              disableTruck(row.orgCode, paramObj, (success, error) => {
                if (success) {
                  this.$message({
                    type: 'success',
                    message: disabled ? '加入黑名单成功' : '移出黑名单成功',
                    duration: 1000
                  });
                  done();
