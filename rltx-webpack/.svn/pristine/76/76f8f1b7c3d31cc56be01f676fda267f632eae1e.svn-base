  ...
  <div id="list" class="list-page">
    <div class="list-wrap">
      <div class="page-header clearfix">
        <h2 class="page-title fl">司机管理</h2>
              ...
              <td class="fix-col">
                <div class="cell" style="width: 100px">
                  <el-select placeholder="请选择" v-model="operValues[list.code]">
                    <el-option v-if="list.disabled === '否'" :key="'冻结'" :label="'冻结'" :value="'冻结'">
                      <div @click="handleDisabled(list, 'true')">
                        <a href="JavaScript:;">冻结</a>
                      </div>
                    </el-option>
                    <el-option v-if="list.disabled === '是'" :key="'解冻'" :label="'解冻'" :value="'解冻'">
...
<script type="text/ecmascript-6">
  ...
  import { getSearch, getColumn, list, disableDriver } from '../../api/DriverManagementService';
  ...
  export default {
    ...
    methods: {
      ...
      getData() {
        ...
        list(this.paramObj, (success) => {
          ...
          tempData.forEach((val) => {
            val.userStatus = val.userStatus === 'activated' ? '已激活' : '未激活';
            val.disabled = val.disabled ? '是' : '否';
          });
          // console.log('listData is', success.content);
      ...
      handleDisabled(row, disabled) {
        ...
        let message = '您确认冻结该司机吗？冻结后，该司机将无法登陆平台';
        if (disabled === 'false') {
          message = '您确认解冻该司机吗？解冻后，该司机将可以正常登陆平台';
        }
        this.$msgbox({
          title: '消息',
          message: h('p', null, [
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              disableDriver(row.code, paramObj, (success, error) => {
                if (success) {
                  this.$message({
                    type: 'success',
                    message: disabled === 'true' ? '冻结成功' : '解冻成功',
                    duration: 1000
                  });
                  done();
