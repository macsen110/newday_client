        ...
        <div class="computed-base-component">
          ...
          <div class="base-list">
            ...
            <div class="base-li">
              ...
              <span class="base-li-value">￥{{submitData.payableAdjustmentQuotaAmount }}元</span>
            </div>
            <div class="base-li">
              <span class="base-li-label">应付调整比例金额总额：</span>
              <span class="base-li-value">￥{{submitData.payableAdjustmentProportionAmount }}元</span>
            </div>
            <div class="base-li">
              <span class="base-li-label">应付运费小计总额：</span>
              <span class="base-li-value">￥{{submitData.payableFreightSubtotalAmount}}元</span>
            </div>
            <div class="base-li end">
              <span class="base-li-label">含税应付运费小计总额：</span>
              <span class="base-li-value">￥{{submitData.taxPayableFreightSubtotalAmount}}元</span>
            </div>
          </div>
        </div>
        <div class="computed-change-component">
          ...
          <div v-if="submitData.adjustmentWay==='quota'">
            <el-form-item label-width="100px" label="应收整单调整">
              <span v-if="!readonly">
                <el-input v-if='!readonly' :disabled="readonly" type="number" @change="onChangeBase" v-model="submitData.payableEntireSingleAdjustment"
                        placeholder="金额"></el-input>
              </span>
              <span v-if="readonly">{{submitData.payableEntireSingleAdjustment}}元</span>      
            </el-form-item>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        formatMoney: {
          adjustQuota: 'payableAdjustmentQuotaAmount',
          billAdjustFee: 'payableAdjustmentProportionAmount',
          confirmedLossDeduction: 'lossDeductionAmount',
          subtotal: 'payableFreightSubtotalAmount',
          taxSubtotal: 'taxPayableFreightSubtotalAmount'
        },
        tempData: {
          willTotal: 0,
          tempTotal: 0,
    ...
    methods: {
      ...
      //      taxPayableFreightSubtotalAmount   含税应收运费小计总额
      //      adjustmentWay 调整方式 quota 按整单定额调整 scale 按整单比例调整
      //          scale->entireSingleAdjustmentRoportion 整单调整比例
      //          quota->payableEntireSingleAdjustment 应收整单调整
      onComputedByMethod() {
        if (this.submitData.adjustmentWay === 'quota') {
          this.submitData.payableAmount = Number(this.submitData.taxPayableFreightSubtotalAmount) - (Number(this.submitData.entireSingleAdjustmentRoportion / 100) * Number(this.submitData.payableAmount));
          this.submitData.entireSingleAdjustmentRoportion = 0;
          this.submitData.payableAmount = Number(this.submitData.taxPayableFreightSubtotalAmount) + Number(this.submitData.payableEntireSingleAdjustment);
        }
        if (this.submitData.adjustmentWay === 'scale') {
          this.submitData.payableAmount = Number(this.submitData.taxPayableFreightSubtotalAmount) - Number(this.submitData.payableEntireSingleAdjustment);
          this.submitData.payableEntireSingleAdjustment = 0;
          this.submitData.payableAmount = Number(this.submitData.taxPayableFreightSubtotalAmount) + (Number(this.submitData.entireSingleAdjustmentRoportion / 100) * Number(this.submitData.payableAmount));
        }
      ...
      onComputedEnd() {
        ...
        this.tempData.proTotal = Number(this.submitData.taxPayableFreightSubtotalAmount);
      ...
      onSetZeroEqEmpty() {
        ...
        this.submitData.payableEntireSingleAdjustment = this.submitData.payableEntireSingleAdjustment || '';
