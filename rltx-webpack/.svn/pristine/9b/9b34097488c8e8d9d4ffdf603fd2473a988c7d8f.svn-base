...
<script type="text/ecmascript-6">
  ...
  import { certOrgDetail, certOrgInfo, certOrgApproval, getImage } from '../../../api/PlatformService';
  ...
  export default {
    ...
    methods: {
      ...
      handleApprovalSuccess() {
        certOrgApproval(this.code, { certStatus: 'authenticated' }, (success, error) => {
      ...
      handleApprovalFail() {
        certOrgApproval(this.code, { certStatus: 'failed', failReason: this.form.failReason }, (success, error) => {
          if (error || success.code !== 200) {
            this.$message({
              type: 'error',
              message: error.content,
    ...
    created() {
      ...
      certOrgDetail((eleConfig) => {
        ...
        if (urlCode) {
          ...
          self.code = urlCode;
          certOrgInfo(paramObj, (success, error) => {
            if (error) {
              self.loading = false;
              self.$message({
                type: 'error',
            ...
            if (success) {
              ...
              // success.content.platformCertOrgAttachmentInfoList.forEach((img) => {
              //   if (img.attachmentName === '营业执照') {
              //     self.$set(self.domainObject, 'businessLicenceCode', img.attachmentResourceCode);
              //   }
              // });
              // 图片数据
              const attachmentList = success.content.platformCertOrgAttachmentInfoList;
