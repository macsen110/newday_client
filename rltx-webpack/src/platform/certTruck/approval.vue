...
<script type="text/ecmascript-6">
  ...
  import { certTruckDetail, certTruckInfo, certTruckApproval, getImage } from '../../../api/PlatformService';
  ...
  export default {
    ...
    methods: {
      ...
      handleApprovalSuccess() {
        certTruckApproval(this.code, { certStatus: 'authenticated' }, (success, error) => {
      ...
      handleApprovalFail() {
        certTruckApproval(this.code, { certStatus: 'failed', failReason: this.form.failReason }, (success, error) => {
          if (error || success.code !== 200) {
            this.$message({
              type: 'error',
              message: error.content,
    ...
    created() {
      ...
      certTruckDetail((eleConfig) => {
        ...
        if (urlCode) {
          ...
          self.code = urlCode;
          certTruckInfo(paramObj, (success, error) => {
            if (error) {
              self.loading = false;
              self.$message({
                type: 'error',
            ...
            if (success) {
              ...
              // success.content.platformCertTruckAttachmentInfoList.forEach((img) => {
              //   switch (img.attachmentName) {
              //     case '道路运输证': self.$set(self.domainObject, 'operateLicenseResourceCode', img.attachmentResourceCode); break;
              //     case '行驶证': self.$set(self.domainObject, 'drivingLicenseResourceCode', img.attachmentResourceCode); break;
              //     default: break;
              //   }
              // });
              // 图片数据
              const attachmentList = success.content.platformCertTruckAttachmentInfoList;
