  ...
  <div id="list" class="list-page">
    ...
    <div class="list-wrap">
      <div class="page-header clearfix">
        <h2 class="page-title fl">车辆审核</h2>
            ...
            <tr v-for="list in listData" :data-error="list.error">
                ...
                <div class="cell" style="width: 100px">
                  <el-select placeholder="请选择" v-model="operValues[list.code]">
                    <el-option v-if="list.certStatus === 'authenticating'" :key="'审核'" :label="'审核'" :value="'审核'">
                      <div @click="openWindow(`${approvalUrl}?code=${list.code}`, '审核车辆')">
              ...
              <td v-for="(column,index) in columnsData">
                <template v-if="column.detailLinkVisiable==='true'">
                  <a href="javascript:;" class="blue" @click="openWindow(`${approvalUrl}?code=${list.code}`, '审核车辆')">
...
<script type="text/ecmascript-6">
  ...
  import { certTruckSearch, certTruckColumn, certTruckList, certTruckDisabled } from '../../../api/PlatformService';
  ...
  export default {
    ...
    data() {
      return {
        ...
        topDistance: '',
        operValues: {},
        approvalUrl: '/platform/cert_truck/approval.html'
      };
    },
    methods: {
      resetScrollBar() {
      ...
      getData() {
        ...
        certTruckList(this.paramObj, (success) => {
      ...
      handleDisabled(row, disabled) {
        ...
        let message = '您确认冻结该车辆吗？冻结后，该车辆将无法派车';
        if (!disabled) {
          message = '您确认解冻该车辆吗？解冻后，该车辆将可以正常派车';
        }
        this.$msgbox({
          title: '消息',
          message: h('p', null, [
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              certTruckDisabled(row.code, paramObj, (success, error) => {
                if (success) {
                  this.$message({
                    type: 'success',
