  ...
  <div id="template" class="form-page">
    <div class="page-header clearfix">
      ...
      <div class="hd-opr fr">
        ...
        <el-button v-show="editable && isEdit && tplName === 'eleLogisticsPrice'" @click="submitForm('domainObject')">保存</el-button>
    ...
    <el-row v-loading.fullscreen.lock="loading" element-loading-text="加载中">
      <el-form :model="domainObject" ref="domainObject" label-width="140px" :inline="false">
        ...
        <ele-logistics-price ref="eleLogisticsPrice" v-if="tplName === 'eleLogisticsPrice'" :code="code" :ratesCodeList="domainObject.ratesCodeList" :ratesCode="domainObject.ratesCode"></ele-logistics-price>
        <el-col :span="24" v-show='editable || !isEdit' class="el-col-button" v-if="!(tplName === 'eleLogisticsPrice' && code)">
    ...
    <!-- logs -->
    <el-tabs type="border-card" v-if="isEdit && uploadUrl && logType && code">
      <el-tab-pane label="变更记录">
        <logs :selectable="false" :operatable="false" :domain="uploadUrl" :type="logType" :code="code" :isAdvanced="false" :needPage="true"></logs>
      </el-tab-pane>
    </el-tabs>
  </div>
</template>
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          const event = new Event('submitDone'),
            extraParams = this.getExtraParams(this.tplName);
          this.domainObject = Object.assign(this.domainObject, extraParams);
          if (this.isEdit) {
