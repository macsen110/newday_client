...
<template>
    <!--<el-dialog title="地图展示" :visible.sync="dialogVisible" size="large" @open="dialogOpen" @close="dialogClose">-->
      <div class="fix-table-wrap">
        <baidu-map :zoom="15" :center="center" class="pop-map" @ready="mapReady" :scroll-wheel-zoom="true">
...
// import Vue from 'vue';
import { getLogisticsTruckList } from '../../../api/waybillService';
...
export default {
  ...
  methods: {
    ...
    getData() {
      if (this.id) {
        getLogisticsTruckList({ code: this.id }, (success) => {
